...
const ViewAllCreatedParcels = () => {
  ...
  const [selectedParcelToEdit, setSelectedParcelToEdit] =
    React.useState<Parcel | null>(null);
  ...
  const {
    ...
    isLoading,
    isError,
  } = useGetMyParcelsQuery(undefined);
  ...
  const handleCancel = async (parcelId: string) => {
    try {
      ...
      const errorMessage =
        error?.data?.message ||
        "Failed to cancel parcel. It might have already been processed.";
      toast.error(errorMessage);
    }
  };
                  ...
                  >
                    <div className="flex flex-col items-center justify-center min-h-[450px] gap-2 py-6">
                      ...
                      <span className="text-base text-gray-500 dark:text-gray-500">
                        You haven’t created any parcels yet. Try updating
