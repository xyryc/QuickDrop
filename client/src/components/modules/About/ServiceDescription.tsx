      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          ...
          <p className="max-w-2xl mx-auto text-[#24252A]/80 dark:text-white/80 mt-4 text-lg font-medium">
            We are dedicated to providing seamless, secure, and swift delivery
            solutions that connect people and businesses.
          </p>
        </div>
        <div className="grid gap-10 md:grid-cols-3">
          {services.map((service, i) => (
            <Card
              key={i}
              className="relative flex flex-col items-center text-center p-8 shadow-2xl border-0 bg-white/95 dark:bg-[#24252A]/90 rounded-2xl transition-all duration-500 scale-105 shadow-3xl group overflow-visible"
            >
              {/* Decorative blurred blob for card */}
              <div
                className="absolute -top-8 -right-8 w-20 h-20 rounded-full blur-xl pointer-events-none transition-all duration-500 group-scale-125"
              ...
              <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-2/3 h-2 rounded-full bg-gradient-to-r from-[#FF771A] via-[#FFB877] to-[#FFE2C6] opacity-70 transition-all duration-500 group-opacity-100 group-w-full"></div>
            </Card>
          ))}
        </div>
      </div>
